...
// Exemple de route GET sur /bookmarks
app.get("/bookmarks", async (req, res) => {
    try {
        // On fait une requête SQL simple pour récupérer toutes les lignes de la table bookmarks
...
// Route PATCH pour modifier la complétude d'une bookmark
// On attend un body JSON avec completed = true/false
app.patch("/bookmarks/:id", async (req, res) => {
    try {
        const {id} = req.params;
        const {completed} = req.body;
        // Mise à jour de la bookmark par son ID
        const result = await pool.query("UPDATE bookmarks SET completed = $1 WHERE id = $2 RETURNING *", [completed, id]);

        if (result.rowCount === 0) {
            return res.status(404).json({error: "bookmark non trouvée"});
        }

        // On renvoie la bookmark mise à jour
