...
async function run() {
    try {
        ...
        const result = await client.query("select bookmarks.id, bookmarks.name, bookmarks.url from bookmarks join topics on bookmarks.topic_id = topics.id where topics.name ilike '%html%';");
        console.log("Résultats :", result.rows);
        await writeFile('test.txt', `-${result.rows[0].name}` )
    } catch (err) {
